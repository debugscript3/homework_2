...
function ShowUI()
    ...
    if faction_id <= 0 then return end
    ...
    if is_leader == true then
        ...
        addEventHandler("onClientGUIClick", ui.btn_accept, function( key )
            ...
            if source == ui.btn_accept then
                ...
                if invited[ target ] and invited[ target ] > getTickCount( ) then 
                    outputChatBox("Игрока можно приглашать не чаще, чем 1 раз в минуту.", 255, 0 ,0) 
                    return false
                end
                ...
                triggerEvent("onPlayerTryAcceptInvite", target, localPlayer)
                invited[ target ] = getTickCount( ) + invite_cooldown
            end
        end)
